...
auto sha_256::rotate_right(uint32_t c, int nb)
{
    return (c >> nb) | ((c << (32 - nb)) & 0xffffffff);
}

auto sha_256::shift_right(uint32_t c, int nb)
{
    return c >> nb;
}

auto sha_256::operator_1(uint32_t nb1, uint32_t nb2, uint32_t nb3)
{
    return nb1 ^ nb2 ^ nb3;
}

auto sha_256::ch_op(uint32_t nb1, uint32_t nb2, uint32_t nb3)
{
    return (nb1 & nb2) ^ (~nb1 & nb3);
}

auto sha_256::maj_op(uint32_t nb1, uint32_t nb2, uint32_t nb3)
{
    return (nb1 & nb2) ^ (nb1 & nb3) ^ (nb2 & nb3);
}

auto sha_256::s1_op(uint32_t nb)
{
    return rotate_right(nb, 6) ^ rotate_right(nb, 11) ^ rotate_right(nb, 25);
}

auto sha_256::s0_op(uint32_t nb)
{
    return rotate_right(nb, 2) ^ rotate_right(nb, 13) ^ rotate_right(nb, 22);
}

std::vector<unsigned char> sha_256::preprocessing(char *input)
{
...
void sha_256::hash(std::vector<unsigned char> vec)
{
...
int main(int argc, char **argv)
{
    ...
    sha_256 sha;
    auto data = sha.preprocessing(argv[1]);
    sha.hash(data);

    std::ostringstream oss;
    for (int i = 0; i < 8; i++)
    {
        oss << std::hex << std::setfill('0') << std::setw(2) << sha.SQUARE_ROOT[i];
    }
